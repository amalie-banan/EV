...
if __name__ == "__main__":
    print("Initialiserer WindmillDataHandler...")
    
    # Tjek om geopandas er installeret
    try:
        import geopandas as gpd
        print("✅ Geopandas er installeret")
    except ImportError:
        print("❌ Geopandas ikke fundet. Installer med: pip install geopandas")
        exit(1)
    
    # Opret handler
    handler = WindmillDataHandler()
    gdf = handler.download_windmill_data()
    print(type(gdf))
    if gdf is not None:
        print("\n" + "="*50)
        print(handler.get_summary(gdf))
        
        # Test position extraction
        positions = handler.get_windmill_positions(gdf)
        print(type(gdf))
        gdf_converted = handler.transform_utm_to_wgs84(gdf)
            
            # Gem med konverterede koordinater
        handler.save_with_coordinates(gdf_converted, "vindmoeller_complete.csv")
